...
export function useCardGrouping(pack: Deck | undefined) {
  ...
  return useMemo(() => {
    const sortedTypes = Array.from(groupedCards.keys()).toSorted((a, b) => {
    ...
    return sortedTypes.map((type) => {
      ...
      const sortedCards = [...cards].toSorted((a, b) => {
        const rarityA = RARITIES.find((r) => r.code === (a.rarity as MtgRarity));
        const rarityB = RARITIES.find((r) => r.code === (b.rarity as MtgRarity));
